<!--
  发布说明模板：由 .github/workflows/build.yml 在发布时替换占位符后使用。
  可用占位符（缺一不可，渲染后若仍残留 {{...}} 工作流会直接失败）：
    {{VERSION}}     版本号 / tag，例如 v1.0.1
    {{COMMIT}}      构建所用提交的短 SHA
    {{BUILD_DATE}}  构建日期（UTC，yyyy-MM-dd）
    {{SIZE}}        附件大小，例如 380,928 字节（372 KiB）
    {{SHA256}}      附件 SHA-256（大写十六进制）
  改这个文件就会改变以后所有版本的发布说明，不需要改工作流。
-->
