...
系统声音与麦克风通常由**不同硬件设备**的晶振驱动，两者的实际采样率会差几 ppm（百万分之几）。
只对齐起点的话，这个差值会**线性累积**成可见的错位：

| 两路时钟差 | 1 小时后 | 3 小时后 |
| --- | --- | --- |
| 20 ppm | 0.07 s | 0.22 s |
| 50 ppm | 0.18 s | 0.54 s |
| 200 ppm | 0.72 s | 2.2 s |

所以长录音（尤其“游戏/视频声音 + 解说”这类把两路合到一个文件的场景）到后半段会逐渐滑开。这点已自动补偿：

1. **采集时量速率**：用该路“实际收到的音频帧数”除以“这些帧对应的设备时间戳跨度”，得到每一路的真实速率
   （我们自己合成的静音不参与，因此不携带时钟信息；累计跨度不足 30 秒则不采信）；
2. **合成时对齐到同一时钟**：以第一路（系统声音）为参考，其他路按 `实测速率 / 标称速率` 做**分数重采样**
   （读取位置以小数步进 + 线性插值；步长≈1.0 ± ppm 量级，插值误差可忽略）；
3. **不需要就不动**：任一路无可靠测量值、或差距小于 1 ppm、或超过 5000 ppm（明显是坏测量）时，
   保持原来的逐帧 1:1 相加 —— 短录音的行为与之前完全一致；
4. 补偿生效时，完成状态会写明：`已补偿两路时钟差 0.0100%（约 0.36 秒/小时）`。

补偿后剩下的误差只是测量噪声（毫秒级），不再是秒级。若两路本来就接在同一块声卡上（共用一个晶振），
实测差值接近 0，程序不会做任何处理。
...
2. **时钟漂移自检**（不需要音频设备，CI 也跑）：造两路各 60 秒、带“尾部标记音”的输入，
   其中第二路按 2000 ppm 的假想时钟差写入（标记故意偏移 5280 帧），验证补偿后两个标记落在同一帧（峰值 ≈ 2×）；
   并有一组“声明时钟相同”的对照用例，确保不补偿时两个标记保持分离；
3. 对每个输出文件做**解码后内容校验**：重新用 Media Foundation 解码生成的 m4a，统计峰值与帧数，
   只有当“源里有信号 ⇒ 输出里也有信号”且时长一致时才判 PASS（避免再出现“尺寸对但内容是静音”的回归）。

日志里每行末尾还会带上 `时钟补偿 X ppm`，方便确认实际有没有做补偿。
...
2. **两路时钟漂移已自动补偿**（见 2.2 节）：以第一路为参考做分数重采样，剩余误差为毫秒级。
   两种情况不补偿、仍为逐帧 1:1：录音太短（测量跨度 < 30 秒）、或某一路几乎没有连续数据包（无法测速）。
3. **相加采用单位增益**：两路同时接近满幅时会限幅（与本项目早期版本及上游 `AudioMixer` 的策略一致），不做自动压缩。4. **加 32 位整数 PCM 格式的设备**：转换分支已修复并有自检覆盖（旧版本对这类设备会算错样本数、读到缓冲区外），
